...
import { requireApiInternalUrl } from "../auth.js";
...
function readContextWindowInputTokens(value: unknown, modelId: string): number {
    if (typeof value === "number" && Number.isInteger(value) && value > 0) {
        return value;
    }

    if (!value || typeof value !== "object" || Array.isArray(value)) {
        throw new Error(`contextWindow is required for model ${modelId}`);
    }
    ...
    if (typeof inputTokens !== "number" || !Number.isInteger(inputTokens) || inputTokens <= 0) {
        throw new Error(`contextWindow.inputTokens is required for model ${modelId}`);
    }
...
/**
 * Fetch contextWindow for a specific model from the API
 * Caches results for 30 minutes to avoid repeated fetches.
 * 
 * Uses /v1/models/{modelId} canonical single-model lookup.
 * 
...
export async function fetchModelContextWindow(modelId: string): Promise<number> {
    ...
    try {
        const response = await fetch(`${requireApiInternalUrl()}/v1/models/${encodeURIComponent(modelId)}`);
        if (!response.ok) {
            console.warn(`[context] Failed to fetch model ${modelId}: ${response.status}`);
            throw new Error(`Failed to fetch model ${modelId}: ${response.status}`);
        }

        const model = await response.json();
        const contextWindow = readContextWindowInputTokens(model?.contextWindow, modelId);
