...
import type { ExecutionRunStateProjection, SSEProgressEvent, StepApprovalDecision } from "../workflow/types.js";
import {
...
const workflowActivities = proxyActivities<typeof import("./activities.js")>({
...
function deriveRunId(input: ExecuteWorkflowWorkflowInput, info: ReturnType<typeof workflowInfo>): string {
...
export async function executeWorkflowWorkflow(input: ExecuteWorkflowWorkflowInput): Promise<WorkflowWorkflowResult> {
    ...
    const expectedWorkflowPrefix = `workflow-${walletAddress}:run:`;
    const isInteractiveRun = Boolean(input.composeRunId);

    if (isInteractiveRun && !info.workflowId.startsWith(expectedWorkflowPrefix)) {
        throw ApplicationFailure.create({
            message: `workflowId mismatch: expected prefix ${expectedWorkflowPrefix}, got ${info.workflowId}`,
            nonRetryable: true,
            type: "ValidationError",
        });
    }

    if (input.workflow?.id && !input.workflow.id.startsWith(`workflow-${walletAddress}`)) {
        throw ApplicationFailure.create({
            message: `workflow payload id mismatch: expected prefix workflow-${walletAddress}, got ${input.workflow.id}`,
            nonRetryable: true,
            type: "ValidationError",
        });
    }
    ...
    try {
        ...
        const orchestratorResult = await executionScope.run(() =>
            workflowActivities.runWorkflowExecutionActivity({
                ...input,
                composeRunId: runId,
        ...
        if (isContinuous && orchestratorResult.success && canContinue) {
            ...
            await continueAsNew<typeof executeWorkflowWorkflow>({
                ...input,
                composeRunId: runId,
...
export async function executeAgentTurnWorkflow(input: ExecuteAgentWorkflowInput): Promise<ExecutionResult> {
    ...
    const expectedWorkflowId = `agent-${input.agentWallet}:thread:${threadId}:run:${input.composeRunId}`;

    if (info.workflowId !== expectedWorkflowId) {
        throw ApplicationFailure.create({
            message: `agent workflowId mismatch: expected ${expectedWorkflowId}, got ${info.workflowId}`,
            nonRetryable: true,
            type: "ValidationError",
        });
    }
