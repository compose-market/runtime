...
import { getWorkflow, resolveAgent } from "../frameworks/runtime.js";
import { WorkflowOrchestrator } from "../workflow/orchestrator.js";
import type { ExecutionRunStateProjection, StepApprovalDecision, StepApprovalRequest, WorkflowStep, ExecutorOptions } from "../workflow/types.js";
import {
...
function resolveWorkflow(input: ExecuteWorkflowWorkflowInput): NonNullable<ExecuteWorkflowWorkflowInput["workflow"]> {
    if (input.workflow) {
    ...
    const workflow = getWorkflow(walletAddress);
    if (!workflow) {
        const error = new Error(`Workflow not found for wallet ${walletAddress}`);
        error.name = "ValidationError";
        throw error;
    }
    ...
    return {
        ...
        name: workflow.title || `Workflow ${walletAddress.slice(0, 8)}`,
...
export async function runWorkflowExecutionActivity(
    input: ExecuteWorkflowWorkflowInput & { approvalTimeoutMs?: number },
): Promise<WorkflowWorkflowResult> {
    // Validate payload sizes per Temporal Cloud best practices
    validatePayloadSize(input, "Workflow execution input");
    ...
    const expectedPrefix = `workflow-${input.walletAddress}`;
    ...
    const heartbeatInterval = startPeriodicHeartbeat({
        runId: composeRunId,
        activity: "runWorkflowExecutionActivity",
    });
