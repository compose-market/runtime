...
function mmrRerankFast(results: SearchResult[], lambda: number): SearchResult[] {
    ...
    const selectedTokenStrings: Set<string> = new Set();
    
    const selectedIds = new Set<string>();
    
    const BATCH_SIZE = 20;
    const COMPARE_LIMIT = 15;

    for (let batch = 0; batch < sorted.length; batch += BATCH_SIZE) {
        const batchEnd = Math.min(batch + BATCH_SIZE, sorted.length);
        
        for (let i = batch; i < batchEnd; i++) {
            const candidate = sorted[i];
            if (selectedIds.has(candidate.id)) continue;

            const candidateTokenStr = tokenStrings.get(candidate.id)!;
            ...
            if (selectedTokenSets.length > 0) {
                ...
                const limit = Math.min(selectedTokenSets.length, COMPARE_LIMIT);
                for (let j = selectedTokenSets.length - limit; j < selectedTokenSets.length; j++) {
                    const sim = jaccardSimilarity(candidateTokens, selectedTokenSets[j]);
                    if (sim > maxSim) maxSim = sim;
                    if (maxSim > 0.8) break;
                }
            }

            if (maxSim < 0.6 || selected.length < 10) {
