...
export async function searchMemoryLayers(params: LayeredSearchParams): Promise<LayeredSearchResult> {
    ...
    if (layers.includes("working")) {
        ...
        const docs = await sessions.find({ agentWallet: params.agentWallet }).sort({ lastAccessedAt: -1 }).limit(limit * 3).toArray();
        const filtered = docs
    ...
    if (layers.includes("scene")) {
        ...
        const docs = await transcripts.find(filter).sort({ createdAt: -1 }).limit(limit * 3).toArray();
        const filtered = docs
            .filter((doc) => {
