...
function buildScopeKey(scope: {
    ...
    return [
        `a:${normalizeScopePart(scope.agentWallet)}`,
        `m:${normalizeScopePart(scope.mode)}`,
        `h:${normalizeScopePart(scope.haiId)}`,
        `u:${normalizeScopePart(scope.userAddress)}`,
        `t:${normalizeScopePart(scope.threadId)}`,
    ].join(":");
}
...
function buildNamespaceChain(scope: {
    agentWallet: string;
    userAddress?: string;
    threadId?: string;
    mode?: "global" | "local";
...
async function getNamespaceToken(scope: {
    agentWallet: string;
    userAddress?: string;
    threadId?: string;
    mode?: "global" | "local";
...
export async function resolveScopedCacheKey(
    key: string,
    scope: { agentWallet: string; userAddress?: string; threadId?: string; mode?: "global" | "local"; haiId?: string },
): Promise<string> {
    // Always namespace-token the key. Without this, memory writes don't
    // invalidate query-cache reads, so the agent re-serves stale results
    // for up to TTL seconds — the user just told the agent something and
    // it's invisible to the next read. That defeats hot-memory UX.
