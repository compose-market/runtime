...
/**
 * Operator-supplied persona override. Identity (name, description, skills, tools, memory
 * discipline) is rendered per-turn from the typed AgentIdentity struct in manowar.ts —
 * this field carries ONLY the custom persona override, if any.
 */
function resolvePersonaOverride(systemPrompt?: string): string | undefined {
...
function buildRuntimeConfig(
    ...
    return {
        ...
        model: params.model,
        plugins,
        systemPrompt: resolvePersonaOverride(params.systemPrompt),
        memory: true,
    };
}
...
function buildRuntimeConfigFromRegistered(agent: RegisteredAgent): AgentConfig {
    return {
        ...
        systemPrompt: resolvePersonaOverride(agent.systemPrompt),
        memory: true,
    };
}
