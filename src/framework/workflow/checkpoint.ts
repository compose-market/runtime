 ...
 * 2. Share experiences between workflows/executions
...
export async function persistCheckpoints(
    ...
    const memoryId = await addMemoryWithGraph({
        messages: [
            { role: "system", content: `Checkpoint summary for ${workflowWallet}` },
...
/**
 * Retrieve past checkpoints from Mem0 for a workflow
...
export async function retrievePastInsights(
    ...
    const results = await searchByEmbedding(workflowWallet, query, limit);
