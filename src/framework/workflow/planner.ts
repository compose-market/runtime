/**
 * Workflow Planner - Task Decomposition & Planning Node
 * 
 * Implements the Plan → Act → Reflect pattern for Workflow orchestration.
 * 
...
export class TaskPlanner {
     ...
     * @param agentCards - Optional agent cards from workflowCard (preferred source for agent info)
     */
    constructor(
        workflow: Workflow,
        plannerModel: string,
        ...
        // Use createModel() for proper provider routing (Fireworks, AI/ML, etc.)
        this.model = createModel(plannerModel, 0.2);
    }

    /**
     * Provide coordinator context (workflow card + workflow graph + user request)
     */
    ...
    /**
     * Get available agents - prefers agentCards from workflowCard (IPFS source of truth),
     ...
     * 
     * @param workflowWallet - Optional wallet for reliability lookups
     */
    private async getAvailableAgentsWithTools(workflowWallet?: string): Promise<{
        name: string;
        description: string;
        capabilities: string[];
        ...
        // Prefer agentCards from workflowCard (contains correct names from IPFS)
        if (this.agentCards?.length) {
            return Promise.all(this.agentCards.map(async (agent) => {
                ...
                if (workflowWallet) {
                    try {
                        const rel = await getAgentReliability(workflowWallet, agent.name);
                        if (rel.totalRuns > 0) {
                            reliability = {
     ...
     * 
     * @param workflowWallet - The workflow wallet for Mem0 lookup
...
Review the past evaluations and suggest improvements for the upcoming execution.`),
    ...
    async createPlan(
        ...
        const agents = await this.getAvailableAgentsWithTools(workflowWallet);
 ...
 * These are added to WorkflowState via extension
 */
