/**
 * Workflow Orchestrator - Simplified Architecture
 * 
 * Implements the simplified orchestration pattern from suggestions.md:
 * - Static system prompt from workflowCard (built once)
...
import { fetchWorkflowCard, buildSystemPromptFromCard, normalizeWorkflowCard, assertWorkflowCard, type WorkflowCard } from "./registry.js";
...
export class WorkflowOrchestrator {
    ...
    private workflowCard: WorkflowCard | null = null;
    ...
    /**
     * Initialize the orchestrator by fetching workflowCard and building system prompt
     */
    async initialize(workflowCardUri?: string): Promise<void> {
        ...
        try {
            ...
            // Fetch workflowCard if URI provided
            if (workflowCardUri) {
                const fetched = await fetchWorkflowCard(workflowCardUri);
                if (fetched) {
                    this.workflowCard = normalizeWorkflowCard(fetched);
                    assertWorkflowCard(this.workflowCard);
                }
                if (this.workflowCard) {
                    this.systemPrompt = buildSystemPromptFromCard(this.workflowCard);
                } else {
    ...
    /**
     * Build system prompt from workflow (fallback if no workflowCard)
     */
  ...
  - Tools: ${Array.isArray(meta.plugins) ? meta.plugins.join(", ") : "none"}`;
        ...
        return `You are the coordinator for workflow "${this.workflowCard?.title}".

## GOAL
${this.workflowCard?.description}

## COMPONENT AGENTS
${this.workflowCard?.agents?.map((agentCard: { name: string; model: string; plugins?: Array<{ name: string }> }, i: number) => {
...
4. Only complete when all tasks are done.`;
    ...
    private buildCoordinationContext(userRequest: string): string {
        const payload = {
            workflow: {
                walletAddress: this.workflowCard?.walletAddress,
                title: this.workflowCard?.title,
                description: this.workflowCard?.description,
                edges: this.workflowCard?.edges || [],
                agents: this.workflowCard?.agents?.map(a => ({
                    name: a.name,
    ...
    async execute(
        ...
        // Ensure initialized
        await this.initialize(options.workflowCardUri);

        if (!this.workflowCard?.walletAddress) {
            throw new Error("[Orchestrator] workflowCard.walletAddress is required - ensure workflowCardUri is provided");
        }
        const walletAddress = this.workflowCard.walletAddress;
        ...
        const expectedPrefix = `workflow-${walletAddress}`;
        ...
        // Pass workflowCard.agents for correct agent names (not fallback workflow step names)
        this.planner = new TaskPlanner(
            this.workflow,
            this.coordinatorModel,
            [this.tokenTracker],
            this.workflowCard?.agents
        );
        this.planner.setWorkflowGraph({
            agents: this.workflowCard?.agents,
            steps: this.workflow.steps,
            edges: this.workflowCard?.edges,
        });

        this.emitProgress("start", { runId, message: "Starting workflow execution" });
        runState = this.emitRunState(options, runState, {
            status: "running",
            progress: 2,
        ...
        try {
            ...
            const edgesContext = this.workflowCard?.edges?.length
                ? `Workflow execution order: ${this.workflowCard.edges.map(e =>
            ...
            const runPlan = async (plan: Awaited<ReturnType<TaskPlanner["createPlan"]>>) => {
                ...
                while (true) {
                    ...
                    let agentCard = this.workflowCard?.agents?.find((a: { walletAddress?: string; name: string }) =>
                    ...
                    if (!agentCard && agentWallet) {
                        agentCard = this.workflowCard?.agents?.find((a: { walletAddress?: string }) => a.walletAddress === agentWallet);
                    }
                    ...
                    const result = await delegatePlanStep(
                        ...
                        },
                        {
                            ...
                            sessionBudgetRemaining: options.payment?.sessionBudgetRemaining ?? null,
                        },
                    );
            ...
            await addMemoryWithGraph({
                messages: [
                    { role: "system", content: `Workflow ${walletAddress} completed` },
            ...
            if (isLangSmithEnabled()) {
                await recordLearning(
                    "workflow-workflow-evaluations",  // dataset name
                    {  // input
                        goal: userRequest,
                        workflowWallet: walletAddress,
                        stepCount: allStepResults.length,
            ...
            const usageRecords = Array.from(this.contextManager.getState().agentUsage.values());
            const inputTokens = usageRecords.reduce((sum, usage) => sum + usage.inputTokens, 0);
            const outputTokens = usageRecords.reduce((sum, usage) => sum + usage.outputTokens, 0);
...
export async function executeWithOrchestrator(
    ...
    const orchestrator = new WorkflowOrchestrator(workflow, coordinatorModel);
