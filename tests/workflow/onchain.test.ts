...
import { fetchAgentOnchain, fetchWorkflowOnchain, buildWorkflowWorkflow } from "../../src/onchain.js";
import { getPublicClient, CHAIN_IDS } from "../../src/chains.js";
import { hasAgent, registerAgent } from "../../src/manowar/runtime.js";
...
describe("On-chain Logic (Multichain Verification)", () => {
    ...
    const originalGateway = process.env.PINATA_GATEWAY_URL;

    beforeEach(() => {
        vi.clearAllMocks();
        process.env.PINATA_GATEWAY_URL = "https://gateway.test/ipfs";
    ...
    afterEach(() => {
        ...
        if (originalGateway === undefined) {
            ...
            process.env.PINATA_GATEWAY_URL = originalGateway;
        }
    });
    ...
    it("should fetch workflow data with mandatory chainId", async () => {
        ...
        mockClient.readContract.mockImplementation(async (args: { functionName: string; args?: bigint[] }) => {
            if (args.functionName === "getWorkflowData") {
                return [
                    "Test Workflow",
                    "Desc",
                    "banner.png",
        ...
        const result = await fetchWorkflowOnchain(chainId, manowarId);
    ...
    it("should build workflow across different chains", async () => {
        ...
        const workflow = await buildWorkflowWorkflow(chainId, manowarId);
