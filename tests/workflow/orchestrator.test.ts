...
import type { Workflow } from "../../src/manowar/workflow/types.js";
...
const mockFetchWorkflowCard = vi.fn(async () => ({
    schemaVersion: "1.0.0",
    title: "Test Manowar",
...
const mockCreateRun = vi.fn((params: { runId?: string; workflowId: string; workflowWallet?: string; input: Record<string, unknown> }) => ({
...
vi.mock("../../src/manowar/workflow/delegation.js", () => ({
    delegatePlanStep: mockDelegatePlanStep,
}));

vi.mock("../../src/manowar/workflow/registry.js", () => ({
    fetchWorkflowCard: mockFetchWorkflowCard,
    buildSystemPromptFromCard: vi.fn(() => "system prompt"),
    normalizeWorkflowCard: vi.fn((card) => card),
    assertWorkflowCard: vi.fn(() => undefined),
    clearCardCache: vi.fn(() => undefined),
    discoverAgentTools: vi.fn(async () => []),
}));

vi.mock("../../src/manowar/workflow/embeddings.js", () => ({
    getRelevantContext: vi.fn(async () => []),
    recordConversationTurn: vi.fn(async () => undefined),
}));

vi.mock("../../src/manowar/workflow/memory.js", () => ({
...
vi.mock("../../src/manowar/workflow/langsmith.js", () => ({
...
vi.mock("../../src/manowar/workflow/checkpoint.js", () => ({
...
vi.mock("../../src/manowar/workflow/run-tracker.js", () => ({
...
vi.mock("../../src/manowar/workflow/agentic.js", () => ({
    isAgenticCoordinatorModel: vi.fn(() => true),
}));

vi.mock("../../src/manowar/workflow/context.js", () => ({
    ContextWindowManager: MockContextWindowManager,
}));

let WorkflowOrchestrator: (new (workflow: Workflow, coordinatorModel: string) => {
    initialize: (workflowCardUri?: string) => Promise<void>;
...
function createTestWorkflow(workflowId: string = `workflow-${TEST_WALLET}`): Workflow {
    return {
        id: workflowId,
        name: "Test Workflow",
...
beforeAll(async () => {
    process.env.API_URL = process.env.API_URL || "https://api.compose.market";
    process.env.RUNTIME_URL = process.env.RUNTIME_URL || "https://runtime.compose.market";
    process.env.RUNTIME_INTERNAL_SECRET = process.env.RUNTIME_INTERNAL_SECRET || "test-secret";

    const orchestratorModule = await import("../../src/manowar/workflow/orchestrator.js");
    WorkflowOrchestrator = orchestratorModule.WorkflowOrchestrator as typeof WorkflowOrchestrator;
    executeWithOrchestrator = orchestratorModule.executeWithOrchestrator;

    const registryModule = await import("../../src/manowar/workflow/registry.js");
...
describe("WorkflowOrchestrator", () => {
    beforeEach(() => {
        ...
        mockFetchWorkflowCard.mockClear();
    ...
    it("initializes with workflow card", async () => {
        const workflow = createTestWorkflow();
        const orchestrator = new WorkflowOrchestrator(workflow, "gpt-4o");

        await orchestrator.initialize("ipfs://test-cid");

        expect(mockFetchWorkflowCard).toHaveBeenCalledTimes(1);
    });
    ...
    it("enforces workflow ID invariant workflow-wallet", async () => {
        const workflow = createTestWorkflow("workflow-0xwrongwallet");
        const orchestrator = new WorkflowOrchestrator(workflow, "gpt-4o");

        await expect(orchestrator.execute("Test request", {
            workflowCardUri: "ipfs://test-cid",
        })).rejects.toThrow(`workflow.id mismatch: expected to start with workflow-${TEST_WALLET}`);
    });
