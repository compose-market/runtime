...
vi.mock("../src/runtimes/transports/http.js", () => ({
  HttpSseClientTransport: MockTransport,
}));

vi.mock("../src/runtimes/transports/docker.js", () => ({
  DockerClientTransport: MockTransport,
}));

vi.mock("../src/runtimes/transports/npx.js", () => ({
...
describe("runtime mcp runtime", () => {
  ...
  afterEach(async () => {
    const module = await import("../src/runtimes/mcp.js");
  ...
  it("reuses cached session when still alive", async () => {
    ...
    const { getServerTools } = await import("../src/runtimes/mcp.js");
  ...
  it("respawns once and retries tool execution on invalid session", async () => {
    ...
    const { executeServerTool, getServerTools } = await import("../src/runtimes/mcp.js");
  ...
  it("throws MCP_CONFIG_NOT_FOUND when server is unknown", async () => {
    ...
    const { getServerTools, McpRuntimeError } = await import("../src/runtimes/mcp.js");
